...
async function resolveWikimediaUrl(url) {
  ...
  try {
    ...
    // From: .../commons/thumb/a/ab/File.jpg/800px-File.jpg
    // To:   .../commons/a/ab/File.jpg
    if (url.includes('/thumb/')) {
        ...
        if (thumbIndex !== -1) {
            ...
            newParts.pop(); // Remove the size-specific part (last part)
...
app.get("/api/proxy-image", async (req, res) => {
  ...
  console.log(`[Proxy] 📥 Request (3001) for: ${imageUrl}`);
  ...
  try {
    ...
    console.log(`[Proxy] ✅ Sent ${response.data.byteLength} bytes from 3001`);
  } catch (error) {
    if (error.response) {
      console.error(`[Proxy] ❌ Source Error (3001): ${error.response.status} for ${imageUrl}`);
      return res.status(error.response.status).send(`Proxy Error: Source returned ${error.response.status}`);
    }
    console.error(`[Proxy] ❌ Network Error (3001): ${error.message}`);
...
async function getUniqueFilePath(baseDir, filename) {
  ...
  let counter = 2;
  while (
    await fs.pathExists(path.join(baseDir, `${filename}_${counter}${ext}`))
  ) {
    counter++;
  }
  ...
  try {
    ...
    for (const noteData of notes) {
      try {
        ...
        errors.push({
          note: noteData.title || noteData.id,
          error: error.message,
        });
      }
    }
...
app.delete("/api/notes/:noteId", async (req, res) => {
  try {
    ...
    console.log(
      `[SERVER] Note not found via direct path. Starting recursive search for ID: ${noteId}...`,
    );
      ...
      for (const file of files) {
        ...
        if (stats.isDirectory()) {
          ...
          try {
            ...
            if (parsed.data.id === noteId) {
              console.log(
                `[SERVER] ✅ Found matching note! Deleting: ${filePath}`,
              );
    ...
    console.log(
      `[SERVER] ℹ️ Note ${noteId} not found on disk after recursive search.`,
    );
...
app.delete("/api/folders/:folderId", async (req, res) => {
  try {
    ...
    const loadAllFolders = async () => {
      ...
      for (const file of files) {
        if (file.endsWith(".json"))
          items.push(await fs.readJson(path.join(fDir, file)));
      }
      return items;
    };
    ...
    if (folder) {
      ...
      console.log(
        `[SERVER] Attempting to delete physical folder: ${physicalPath}`,
      );
...
app.post("/api/questions", async (req, res) => {
  try {
    ...
    console.log(
      `[SERVER] Saved questions: ${data.questions?.length || 0} questions, ${data.folders?.length || 0} folders`,
    );
  ...
  try {
    ...
    if (!API_KEY) {
      ...
      return res.status(500).json({
        ...
        details:
          "The server is missing the OPENROUTER_API_KEY in the .env file. Please check server/.env",
      });
    }
    ...
    console.log(
      `[AI] Request size: ${JSON.stringify(messages).length} chars, max_tokens: ${max_tokens}`,
    );
...
const serverInstance = app.listen(PORT, "0.0.0.0", () => {
  console.log("==================================================");
  console.log(
    `[${new Date().toLocaleTimeString()}] NOTES SERVER UPDATED (v3.0 AI mode integration)`,
  );
  console.log(
    `Running on http://0.0.0.0:${PORT} (Accessible on your local network)`,
  );
  console.log(`Storage: ${NOTES_BASE_DIR}`);
  if (process.env.OPENROUTER_API_KEY) {
    console.log(
      `[OK] AI PROXY CONNECTED (Key: ${process.env.OPENROUTER_API_KEY.substring(0, 8)}...)`,
    );
  } else {
    console.warn(
      `[WARNING] AI PROXY DISABLED (OPENROUTER_API_KEY missing in .env)`,
    );
  }
  console.log("==================================================");
...
serverInstance.on("error", (err) => {
  if (err.code === "EADDRINUSE") {
    ...
    console.error(
      `Please close any existing command prompt windows running the server.`,
    );
    console.error(
      `If you cannot find the window, restart your computer or kill the 'node' process.\n`,
    );
  } else {
